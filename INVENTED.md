...
| `field-rgb — a custom property set by grid.js, card.js and detail.js` | `the category accent, as an r,g,b triplet` | Which brand colour a chapter, a card and a product view are painted in. Set from the category menu.json already carries, so the four chapters cannot drift apart from the four chips. It is a triplet rather than a colour so the same token can be used at an alpha. |
...
**How it was decided.** Page 17 is the origin: its three card levels measure 1 : 0.72 : 0.50, so --fs-item is the '1' and the card's second level is derived from it with calc() rather than carrying its own clamp — which keeps the measured 0.72 true at every viewport width instead of only at the top of a clamp. The levels above the card continue the same 1.389 step upward. Two of the sizes below are forced by the contrast table rather than chosen: white on hot pink is 4.06 and hot pink on cream is 3.85, both legal as large text only, and WCAG's large-text floor is 14pt bold = 18.67px. The eight sizes below the card levels are the approved design’s own display sizes, transcribed from site/.site/b.html clamp for clamp rather than re-derived. They are fluid rather than steps of a ratio because the design uses its headings as objects: the chapter head is 96px at 1600 and 34px at 320, which no single ratio expresses. The middle term of each is a bare vw, as the approved page has it — an earlier transcription wrote them as `Xrem + Yvw`, which reaches the floor sooner but is a different size at every width in between, and drew the section head 43% too large at 768.

| Token | Value | Note |
| --- | --- | --- |
| `--t-display` | `clamp(2.75rem, 1.915rem + 3.43vw, 5rem)` | Hero. 44px at 390, 80px at 1440. One step above the section size. |
| `--t-deck` | `1.5rem` | 24px flat, at every width. Regular-weight cream on hot pink is 4.06:1, which WCAG allows at 24px and forbids below it — so this is the contrast floor and the approved page’s own value at the same time. |
...
| `--t-h-section` | `clamp(1.75rem, 4.4vw, 4.125rem)` | A section head on a cream or light field — "CUATRO MANERAS DE ENTRAR". 28px at 320, 66px at 1500 and above. |
| `--t-h-big` | `clamp(1.875rem, 5.1vw, 5rem)` | The two story beats’ own heads — "AQUÍ CABE TODO EL ANTOJO". Also the VISIT US head, which the approved page draws 1.4px smaller at 1440; one token rather than two for a difference nobody can see. |
| `--t-h-menu` | `clamp(2rem, 5.6vw, 5.5rem)` | The menu’s own head — "EL MENÚ COMPLETO". One step above a section head, because it opens the half of the page the customer came for. |
| `--t-h-chapter` | `clamp(2.125rem, 6vw, 6rem)` | A chapter head standing on its own colour field — HELADOS, ANTOJITOS, POSTRES, BEBIDAS. Painted cream on a brand colour, so it is far above the 24px large-text floor at every width. |
| `--t-h-hero` | `clamp(2.125rem, 6.1vw, 6.5rem)` | The headline the client asked to keep, ASÍ SABE LA ALEGRÍA. The largest size on the page; cream on hot pink, which is 4.06:1 and therefore legal only as large text. |
| `--t-h-beat` | `clamp(1.4375rem, 2.9vw, 2.75rem)` | The numbered heads inside the second story band. They sit inside a cream reading card, not on the blue field, so they are free of the large-text floor. |
| `--t-kicker` | `clamp(0.6875rem, 1.05vw, 0.875rem)` | The small tracked label above a head — "LA TIENDA", "CUATRO CAPÍTULOS". Never painted in a brand colour on a brand field: it always gets its own cream pill or a cream ground, because at 11–14px it is far below the large-text floor. |
| `--t-prose` | `clamp(0.9375rem, 1.35vw, 1.25rem)` | Body copy in the story bands and the category panels. The approved page sets prose per section between 1.22vw and 1.5vw; this one token sits in the middle of that band and lands within 1.5px of every one of them. |
...
**How it was decided.** The width is the measure the approved design was drawn to — four product cards on a chapter field, with the display case and the typographic statement running the same width. The gutter is fluid so a phone loses margin before it loses card.

| Token | Value | Note |
| --- | --- | --- |
| `--maxw` | `1480px` | Content width. Four cards plus gutters at 1440, and the measure the approved design (site/.site/b.html) was drawn at. Was 1320px before the design was replaced. |
| `--gutter` | `clamp(16px, 4vw, 64px)` | Page margin. |
...
| Token | Value | Note |
| --- | --- | --- |
| `cols — a custom property set inline by detail.js` | `2 or 3, from the photograph count` | How many alternate photographs a product view lays out across. Two at four photographs, three above that. The menu grid itself no longer carries a column count: the approved design sizes its cards with auto-fit, so the column count is whatever fits. |
| `nav-h — a custom property measured and set by grid.js` | `64px, then measured` | A fallback only. grid.js replaces it with the header's real rendered height, so the sticky category bar cannot drift when the header changes. |
| `bar-h — a custom property set by grid.js` | `60px` | Height of the sticky category bar, which the section headings offset against. |
...
**Why this had to be decided.** brand.json typography.display asks for 'a bold, rounded, playful sans-serif' for display and headers and names Fredoka and Baloo as REFERENCES. The brand kit's own wording is 'Fredoka, Baloo, or a comparable rounded display typeface', so it does not lock one. Picking a specific face is therefore this website's decision, not the client's.

**How it was decided.** Fredoka was taken because it is the first face the kit names, it is a variable font (300-700, one file for every weight this site uses), and its latin subset is 29 kB self-hosted. Baloo would have been equally compliant. brand.json records the pick as typography.display.chosen; this page is the disclosure that goes with it. If the client wants Baloo, or a face of their own, the answer belongs in brand.json and this site follows it.

| Token | Value | Note |
| --- | --- | --- |
| `typography.display.chosen` | `Fredoka` | Not a CSS token — the choice itself. --font-display, --w-display and --w-semi in tokens.css are all read out of brand.json and are not inventions. |
| `fonts.css @font-face 'Fredoka'` | `weight 300 700, latin + latin-ext` | Self-hosted from public/fonts. The cyrillic subset is on disk and deliberately not declared: nothing here is set in Cyrillic. |

## The approved design's own values

**Why this had to be decided.** The design the client approved on 2026-08-25 (site/.site/b.html) is drawn with tracking, small fixed type sizes, corner radii and animation periods. brand.json publishes NONE of these: typography.sizes and typography.lineHeights are null, letter-spacing is never mentioned, visualDirection.cornerRadius is null, and motion is in notSpecifiedInSource. Every value here is therefore a decision made to ship this page.

**How it was decided.** Read off the approved page and promoted to tokens so there is one name per role rather than the same number typed in nine files. Nothing was re-tuned in the move: the values are what the client looked at and said yes to.

| Token | Value | Note |
| --- | --- | --- |
| `--tr-caps` | `0.06em` | Tracking for the smallest all-caps labels. Same value as --tracking-caps, under a name that matches the other three. |
| `--tr-wide` | `0.14em` | Tracking for header links and chips. |
| `--tr-wider` | `0.20em` | Tracking for section kickers and the display-case tags. |
| `--tr-widest` | `0.24em` | Tracking for the widest kickers and the ticker. |
| `--tr-display` | `-0.012em` | Negative tracking on Fredoka headings, which are drawn loose at display size. |
| `--tr-slab` | `-0.022em` | Tighter still, for the four-line typographic statement where the words are set as one block. |
| `--fs-tiny` | `11px` | The smallest label on the page — the card's photo count and the display case's tag at its floor. |
| `--fs-chip` | `12px` | Chip and column-heading size. |
| `--fs-tag` | `13px` | Header links, jump links and footer links at their floor. |
| `--fs-lede` | `19px` | The hero eyebrow and the shop-name rail. Deliberately at or above 18.66px so bold cream on hot pink is legal at 3:1 rather than needing 4.5:1. |
| `--r-sign` | `clamp(20px, 3vw, 44px)` | The hero sign's corner radius. |
| `--r-panel-lg` | `26px` | Category panels, story beats. |
| `--r-tile-lg` | `20px` | Product cards and fact panels. |
| `--drip-h` | `6px` | The height of the flat part of a drip edge; the lobes hang below it. |
| `--bulb-cycle` | `2.6s` | One pass of the marquee bulbs around the sign. |
| `--tick-cycle` | `68s` | One pass of the product-name ticker. |
| `--case-cycle` | `74s` | One pass of the display case. |
| `--dust-cycle` | `56s` | One pass of the dot field behind the sign. |
| `--bob-cycle` | `7s` | The base period of a floating cut-out; every second and third one is offset from it. |
| `m — a custom property set per line by build-site.mjs` | `1 / 1.186 / 1.261 / 1.354` | Per-line multipliers on the four-word typographic statement. The ratio of each word's measured advance width in Fredoka 700 to the widest of them, so the four words stack as one solid block instead of a ragged centred list. |
| `--sayw` | `clamp(38px, 16vw, 244px)` | The common width the four statement lines are solved against. |
| `--tint` | `0.10-0.34 alpha of the chapter's accent` | The wash behind a product on its card, so a cut-out stands on the chapter's colour rather than on bare cream. |
| `--panel-body` | `cream, or deep purple on the yellow panel` | The reading strip across a category panel. brand.json visualDirection: keep menu text on cream reading areas. |
| `--panel-fg` | `the category's own accent` | The category name inside that strip. |
| `--panel-go` | `deep purple, or cream on the purple strip` | The 'see Helados' line. Separate from --panel-fg because hot pink on cream is 3.85:1 and that line is 13px, which needs 4.5:1. |
| `--panel-dim` | `0.76 alpha of the strip's ink` | The item count under the name. |
| `--panel-k` | `0.72 alpha of the strip's ink` | The 01-04 numeral above the name. |
| `--bay` | `clamp(104px, 12.1vw, 186px)` | One slot of the drifting display case. Everything in the case — its height, the shelf line, each photograph — is a multiple of this. |
| `--k` | `1, 1.2 above 1900px, 0.66 below 900px, 0.46 below 520px` | The scale of a drip lobe. The lobes are drawn from one measured melt profile and this is the only thing that changes with the viewport. |
| `--stage-h` | `116cqw` | The height of a card's coloured field, as a share of the card's own width. |
| `CASE_W (build-site.mjs)` | `1.72` | How wide a photograph in the display case may be, in bay widths. Above 1 on purpose: the case's products are cropped by their neighbours and by both ends of the frame. |
| `CASE_H (build-site.mjs)` | `1.56` | How tall, in bay widths, against a bay that is 2 tall. |
| `CASE_CAP (build-site.mjs)` | `1.92` | The ceiling on a case photograph's square, so a very thin product cannot burst the case. |
